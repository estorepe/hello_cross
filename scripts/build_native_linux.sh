...
if [ "$ARCH" = "arm64" ]; then
    ...
    cd native/rust && rustup target add aarch64-unknown-linux-gnu
    cargo build --release --target aarch64-unknown-linux-gnu && cp target/aarch64-unknown-linux-gnu/release/hello_rust ../../assets/native/linux/$ARCH/
